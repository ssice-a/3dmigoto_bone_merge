...
def write_simple_override_ini(
    ...
    *,
    ...
    return write_simple_override_ini_from_runtime(output_directory, runtime_plan, ini_file_name=ini_file_name)
        ...
        records.append(
            {
                ...
                "capture_flag": _CAPTURE_FLAG_MAIN,
                "capture_source_count": len(used_indices),
                "capture_pairs": pairs,
            }
        )
        ...
        records.append(
            {
                ...
                "dispatch_rows": len(pairs) * 3,
                "capture_flag": _CAPTURE_FLAG_LOD,
                "capture_source_count": int(lod_record.get("lod_local_bone_count", 0) or 0),
                "capture_pairs": pairs,
            }
        )
...
def _materialize_capture_bone_map_resources(
    ...
    for role, records in (("main", main_records), ("lod", lod_records)):
        for record in records:
    ...
    for key in sorted(grouped):
        bucket = grouped[key]
        roles = set(bucket.get("roles", set()) or set())
        records = list(bucket.get("records", []) or [])
        pairs = _merged_capture_pairs(records)
        if not pairs:
            continue
        source_count = max((int(record.get("capture_source_count", 0) or 0) for record in records), default=0)
        capture_flag = _capture_resource_flag(roles)
        suffix = _capture_resource_suffix(key, roles)
        resource_name = f"ResourceCaptureBoneMap_{suffix}"
        file_name = f"{suffix}-CaptureBoneMap.buf"
        file_path = write_uint32_buffer(
            os.path.join(buffer_dir, file_name),
            _single_capture_bone_map_uints(pairs, source_count=source_count, capture_flag=capture_flag),
        )
        payload = _resource_file_payload(
            file_path,
            output_directory,
            _CAPTURE_BONE_MAP_HEADER_UINTS + len(pairs) * _CAPTURE_BONE_PAIR_STRIDE,
        )
        payload.update(
            {
        ...
        for record in records:
            record["capture_resource_name"] = resource_name
            record["capture_resource_suffix"] = suffix
            record["capture_file_name"] = file_name
            record["capture_file_path"] = file_path
            record["capture_filename"] = payload["filename"]
            record["capture_pair_count"] = len(pairs)
    return resources


def _remove_legacy_capture_bone_maps(buffer_dir: str) -> None:
    for file_name in _LEGACY_CAPTURE_BONE_MAP_FILES:
        path = os.path.join(buffer_dir, file_name)
        if os.path.exists(path):
            os.remove(path)


def _single_capture_bone_map_uints(
    pairs: list[tuple[int, int]],
    *,
    source_count: int,
    capture_flag: int,
) -> list[int]:
    values = [len(pairs), int(source_count), int(capture_flag), 0]
    for source_local_bone, canonical_global_bone in pairs:
        values.extend([int(source_local_bone), int(canonical_global_bone)])
    return values


def _merged_capture_pairs(records: list[dict]) -> list[tuple[int, int]]:
    pairs: list[tuple[int, int]] = []
    seen: set[tuple[int, int]] = set()
    for record in records:
        for source_local_bone, canonical_global_bone in record.get("capture_pairs", []) or []:
            pair = (int(source_local_bone), int(canonical_global_bone))
            if pair in seen:
                continue
            seen.add(pair)
            pairs.append(pair)
...
def _capture_resource_flag(roles: set[str]) -> int:
    if "main" in roles and "lod" in roles:
        return _CAPTURE_FLAG_MAIN_LOD
    if "lod" in roles:
        return _CAPTURE_FLAG_LOD
    return _CAPTURE_FLAG_MAIN


def _capture_resource_suffix(key: tuple[str, int, int], roles: set[str]) -> str:
    role_suffix = "MAIN_LOD" if "main" in roles and "lod" in roles else ("LOD" if "lod" in roles else "MAIN")
    return _safe_suffix(f"{key[0]}_{key[2]}_{key[1]}_{role_suffix}")
...
def _resource_sections(runtime_plan: dict) -> list[str]:
    ...
    capture_map_payloads = list(buffers.get("capture_bone_maps", []) or [])
    if capture_map_payloads:
        for payload in capture_map_payloads:
            lines.extend(_buffer_resource(str(payload.get("resource_name", "") or ""), payload))
            lines.append("")
        if lines and lines[-1] == "":
            lines.pop()
    else:
        lines.append("; no capture maps")
    lines.extend(
        [
            "",
            "; -------------------------------------------------",
...
def _slot_sort_key(slot_name: str) -> tuple[int, str]:
    value = str(slot_name or "").lower()
    if value.startswith("vb") and value[2:].isdigit():
        return int(value[2:]), value
    return 9999, value


def _texture_override_sections(runtime_plan: dict) -> list[str]:
    capture_by_key: dict[tuple[str, int, int], dict[str, list[dict]]] = {}
    for record in runtime_plan.get("capture_records", []) or []:
        key = _override_key(record)
        capture_by_key.setdefault(key, {"main": [], "lod": []})["main"].append(dict(record))
    for record in runtime_plan.get("lod_capture_records", []) or []:
        key = _override_key(record)
        capture_by_key.setdefault(key, {"main": [], "lod": []})["lod"].append(dict(record))
