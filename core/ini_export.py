...
_MAIN_CAPTURE_RECORDS_FILE = "MainCaptureRecords.buf"
_MAIN_CAPTURE_SOURCE_LOCAL_BONES_FILE = "MainCaptureSourceLocalBones.buf"
_LOD_CAPTURE_RECORDS_FILE = "LodCaptureRecords.buf"
_LOD_CAPTURE_SCATTER_PAIRS_FILE = "LodCaptureScatterPairs.buf"
...
def materialize_bonestore_runtime(
    ...
    capture_meta_path = write_uint32_buffer(
        os.path.join(buffer_dir, _MAIN_CAPTURE_RECORDS_FILE),
        capture_meta or [0, 0, 0, 0],
    )
    capture_indices_path = write_uint32_buffer(
        os.path.join(buffer_dir, _MAIN_CAPTURE_SOURCE_LOCAL_BONES_FILE),
        capture_local_indices or [0],
    )
    lod_meta_path = write_uint32_buffer(
        os.path.join(buffer_dir, _LOD_CAPTURE_RECORDS_FILE),
        lod_meta or [0, 0, 0, 0],
    )
    lod_pairs_path = write_uint32_buffer(
        os.path.join(buffer_dir, _LOD_CAPTURE_SCATTER_PAIRS_FILE),
        lod_pairs or [0, 0],
    )
...
def build_bonestore_ini_content(runtime_plan: dict) -> str:
    ...
    lines.extend(
        [
            ...
            "; Runtime schema v2:",
            ";   main capture: record index -> MainCaptureRecords + MainCaptureSourceLocalBones",
            ";   LOD capture:  record index -> LodCaptureRecords + LodCaptureScatterPairs",
...
def _resource_sections(runtime_plan: dict) -> list[str]:
    ...
    return [
        ...
        "; -------------------------------------------------",
        *_buffer_resource("ResourceMainCaptureRecords", buffers.get("capture_meta", {})),
        "",
        *_buffer_resource("ResourceMainCaptureSourceLocalBones", buffers.get("capture_local_indices", {})),
        "",
        *_buffer_resource("ResourceLodCaptureRecords", buffers.get("lod_capture_meta", {})),
        "",
        *_buffer_resource("ResourceLodCaptureScatterPairs", buffers.get("lod_capture_pairs", {})),
        "",
        "; -------------------------------------------------",
...
def _custom_shader_sections() -> list[str]:
    return [
        ...
        "cs-t0 = vs-t0",
        "cs-t1 = ResourceDumpedCB1_SRV",
        "cs-t2 = ResourceLodCaptureRecords",
        "cs-t3 = ResourceLodCaptureScatterPairs",
        "cs-u1 = ResourceFakeT0_UAV",
        "dispatch = 64, 1, 1",
        "cs-u1 = null",
        "cs-t0 = null",
