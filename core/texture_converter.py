"""Texture conversion helpers used by Blender preview and BMC export."""
...
CACHE_ROOT = PLUGIN_ROOT / ".bmc_cache" / "textures"
...
def find_texconv() -> Path | None:
    env_path = os.environ.get("BMC_TEXCONV") or os.environ.get("MODIMP_TEXCONV")
    if env_path:
...
def _run_texconv(args: list[str]) -> subprocess.CompletedProcess[bytes]:
    ...
    if texconv is None:
        raise TextureConversionUnavailable(
            ...
            f"{BUNDLED_TEXCONV}, or set BMC_TEXCONV."
        )
    try:
...
def load_image_for_blender(source_path: str | Path, *, color_space: str | None = None):
    ...
    if converted_preview is not None:
        image["bmc_original_texture_path"] = str(source)
        image["bmc_converted_preview_path"] = str(converted_preview)
...
def blender_image_export_source(image) -> str:
    ...
    original_path = str(image.get("bmc_original_texture_path", "") or "").strip()
    preview_path = str(image.get("bmc_converted_preview_path", "") or "").strip()
