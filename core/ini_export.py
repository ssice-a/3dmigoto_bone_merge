...
from ..constants import BONESTORE_INI_FILE_NAME, LOCAL_PREVIOUS_ROW_OFFSET
from .models import LocalPaletteRecord, PartRecord


def build_bonestore_ini_content(part_records: list[PartRecord], local_palette_records: list[LocalPaletteRecord]) -> str:
    effective_palette_records = _build_effective_palette_records(part_records, local_palette_records)
    lines: list[str] = []
    ...
    lines.append("")
    lines.extend(_shader_override_sections(part_records))
    lines.append("")
...
def write_bonestore_ini(
    ...
    with open(ini_path, "w", encoding="utf-8", newline="\n") as file_handle:
        file_handle.write(build_bonestore_ini_content(part_records, local_palette_records))
    return ini_path
...
def _local_palette_data_line(local_palette_record: LocalPaletteRecord) -> str:
    if local_palette_record.file_name:
        return f"filename = Buffer/{local_palette_record.file_name}"
    palette_values = " ".join(str(int(value)) for value in local_palette_record.palette_values)
    return f"data = R32_UINT {palette_values}"
