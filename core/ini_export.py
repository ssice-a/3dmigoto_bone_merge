...
from .models import LocalPaletteRecord, LodRuntimePartRecord, PartRecord
...
def build_bonestore_ini_content(
    ...
    lod_runtime: dict | None = None,
    namespace: str | None = None,
) -> str:
    unique_part_records = _deduplicate_part_records(part_records)
    lod_capture_parts = list((lod_runtime or {}).get("capture_parts", []) or [])
    ...
    lines.append("; Stage 1: capture original per-IB palettes into the shared native capture store")
    lines.append("; Stage 2: gather the current consuming palette into a local small buffer")
    lines.append(";")
    lines.append("; IMPORTANT:")
    lines.append(";   ResourceBoneMeta_*.x         = capture_store_base for one source/native palette")
    lines.append(";   ResourceBoneMeta_*.y         = source/capture bone count for that palette")
    ...
    lines.append("")
    lines.extend(_shader_override_sections(unique_part_records, lod_capture_parts))
    lines.append("")
    ...
    palette_by_identity_key = {
        (record.ib_hash, record.match_index_count, record.chunk_index, str(record.variant_id or "")): record
        for record in effective_palette_records
    }
    ...
    for lod_capture_part in lod_capture_parts:
        local_palette_record = palette_by_identity_key[
            (
                lod_capture_part.ib_hash,
                lod_capture_part.match_index_count,
                lod_capture_part.draw_index,
                f"{lod_capture_part.variant_id}__native",
            )
        ]
        lines.extend(_lod_part_sections(lod_capture_part, local_palette_record))
        lines.append("")
...
def write_bonestore_ini(
    ...
    runtime_palette_records = list(local_palette_records) + list((lod_runtime or {}).get("palette_records", []) or [])
    materialized_palette_records = _materialize_local_palette_buffers(
        output_directory,
        part_records,
        runtime_palette_records,
    )
    with open(ini_path, "w", encoding="utf-8", newline="\n") as file_handle:
        file_handle.write(
            build_bonestore_ini_content(
                part_records,
                materialized_palette_records,
...
def _shader_override_sections(
    part_records: list[PartRecord],
    lod_capture_parts: list[LodRuntimePartRecord],
) -> list[str]:
    lines: list[str] = []
    ...
    for lod_capture_part in lod_capture_parts:
        vs_hash = lod_capture_part.vs_hash.lower()
        if vs_hash in seen_hashes:
            continue
        seen_hashes.add(vs_hash)
        lines.extend(
...
def _shared_resource_sections() -> list[str]:
    return [
        ...
        "",
        "; -------------------------------------------------",
        "; Stage 1 shaders: extract -> store into shared native-capture FakeT0",
        "; -------------------------------------------------",
        ...
        "; Redirect cb1[5].x/.y to the shared local gathered buffer.",
        ...
        "",
        "; Local palette resources are generated per IB or per LOD variant below.",
        "; ResourceBoneMeta_*.x/.y store one source palette's capture_store_base and source bone count.",
        "; ResourceLocalPaletteMeta_*.x stores the local palette bone count used by GatherBones.",
    ]
...
def _build_effective_palette_records(
    ...
    records_by_identity = {
        (record.ib_hash.lower(), int(record.match_index_count), int(record.chunk_index), str(record.variant_id or "")): record
...
def _lod_part_sections(capture_part: LodRuntimePartRecord, local_palette_record: LocalPaletteRecord) -> list[str]:
    return [
        f"[ResourceBoneMeta_{capture_part.resource_suffix}]",
        "type = Buffer",
        "format = R32_FLOAT",
        f"data = {float(capture_part.capture_store_base):.1f} {float(capture_part.bone_count):.1f}",
        "",
        f"[TextureOverride_IB_{capture_part.ib_hash.lower()}_merge_{capture_part.variant_id}_{int(capture_part.draw_index)}]",
        f"hash = {capture_part.ib_hash.lower()}",
        f"match_index_count = {int(capture_part.match_index_count)}",
        "match_priority = -500",
        "if vs == 200",
        "  run = CustomShader_ExtractCB1",
        f"  cs-t2 = ResourceBoneMeta_{capture_part.resource_suffix}",
        "  run = CustomShader_RecordBones",
        "endif",
        "; Stage 2 palette: LOD source native local order routed into the shared capture store",
        "run = CustomShader_ExtractCB1",
        f"cs-t2 = ResourceLocalPalette_{local_palette_record.resource_suffix}",
        f"cs-t3 = ResourceLocalPaletteMeta_{local_palette_record.resource_suffix}",
        "run = CustomShader_GatherBones",
...
def _is_default_part_palette(part_record: PartRecord, local_palette_record: LocalPaletteRecord) -> bool:
    ...
    return (
        not str(local_palette_record.variant_id or "")
        and local_palette_record.ib_hash.lower() == part_record.ib_hash.lower()
